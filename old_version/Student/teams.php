...
{
	echo "<script>alert('Database error. Please email admin at emu_invent@emich.edu!')</script>";
}
...
echo "<script type='text/javascript'>window.location.href = 'home.php';</script>";	
	...
	echo "<script>alert('Submission deadline is 02/26/2023 12:00:00')</script>";
	echo "<script type='text/javascript'>window.location.href = 'home.php';</script>";	
	} 
	}
  #echo "<script type='text/javascript'>window.location.href = 'home.php';</script>";  
  ...
  <form method="post" enctype="multipart/form-data">
                ...
                <tr>
                    <td colspan="6">
                     <br>
						<div >
                        <h6>Please check if your team is already been created! If not, please select "Not applicable" to proceed further</h6>    
...
<div id="London" class="tabcontent" style="<?php if($team_select > 0 ) { echo 'display:block'; } if(($team_submit) > 0) { echo 'display:none' ;}   ?>">
  ...
  <form method="post" enctype="multipart/form-data">
				...
				<div>
									...
									<?php 
									$q_m = mysqli_query($con,"select tbl_student_mentor.student_id as id, tbl_user.first_name, tbl_user.last_name 
from tbl_user, tbl_student_mentor, tbl_team_member 
where tbl_user.id = tbl_student_mentor.student_id 
and tbl_student_mentor.mentor_id = $mentor_id and tbl_student_mentor.student_id = tbl_team_member.student_id 
and (tbl_team_member.team_id is null or tbl_team_member.team_id in (select id from tbl_team where deleted = 1))");
