...
if($list){
	...
	{
	//Get all state data
    $query2 = mysqli_query($con,"SELECT * FROM tbl_team WHERE id not in ($list) and category = '$cat' and year = '$year' and deleted = 0 ORDER BY project_team_name ASC");
    #$q = "SELECT * FROM tbl_team WHERE id not in ($list) and category = '$cat' ORDER BY project_team_name ASC";
    //Count total number of rows
   
        echo '<option value="">Select Team-6'.$q.'</option>';
