...
<div id="wrapper">
	...
	$q_c = mysqli_query($con,"select count(user_id) as count from tbl_judge_assessment 
				...
				if($list!=='')
				{
				$q = "select t.user_id,judge_name,email, group_concat(category) as category, GROUP_CONCAT(project_team_name) as project_team_name from 
				(select tj.user_id,concat(tu.first_name,' ', tu.last_name) as judge_name,tu.email, tt.project_team_name, tt.category FROM tbl_user tu, tbl_judge_team tj, tbl_team tt
										where tj.user_id in ($list) and tj.team_id not in (select team_id from tbl_judge_assessment where user_id in ($list))
...
where tj.user_id in ($list) and ( tj.team_id not in (select team_id from tbl_judge_assessment where user_id in ($list)) or tj.team_id in (select team_id from tbl_judge_assessment where identifying_understanding is null
                 or ideating is null
