...
$current_year=date('Y');
 if ($category !== '') {
  ...
  FROM tbl_team tt WHERE category = '$category' and year='$current_year' and deleted=0 ");
} else {
  $q_team = mysqli_query($conn, "SELECT DISTINCT tt.id AS team_id, tt.project_team_name AS project_name, tt.project_description, tt.category, tt.video_pitch, tt.log_book
  FROM tbl_team tt  WHERE year='$current_year' and deleted=0");
}
          ...
          <table class="table team_table table1">
            ...
            ?>
              ...
              <tr>
                ...
                <td class="description"><a href="viewteams.php?team_id=<?php echo $r_team["team_id"]; ?>"><?php echo $r_team["project_description"]; ?></a></td>
